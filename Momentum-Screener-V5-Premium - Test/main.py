...
def scan_premium_zone_job(cfg: Dict, job_id: str) -> List[Dict]:
    ...
    for sig in gate2:
        ...
        vol_label = (
            f"{vol_surge_ratio:.2f}x"
            if vol_surge_ratio is not None else "N/A"
        )
        is_vol_surge = vol_surge_ratio is not None and vol_surge_ratio >= min_vol_surge

        job_log(job_id,
            f"{ticker} volume surge: {vol_label}  "
            f"({'✓ HIGH' if is_vol_surge else '⚠ moderate/low'}, threshold {min_vol_surge}x)",
        ...
        # ── Premium Score ─────────────────────────────────────────────────────
        # ATM LTP is the richest premium; vol surge amplifies it.
        # Score = vol_surge_ratio × ATM_LTP.  Use 1.0 as fallback vol ratio
        # so even low-vol days score by their raw ATM premium.
        atm_ltp       = ladder[0]["CE_ltp"]
        premium_score = round((vol_surge_ratio or 1.0) * atm_ltp, 2)
    ...
    job_log(job_id,
        f"Premium Zone done — {len(results)} candidate(s) found",
        "info")
