...
DEFAULT_CONFIG = {
    ...
    # Strike quality filters
    "min_ce_oi":                   500,     # minimum open interest on the short CE leg
    "min_ce_ltp":                  10.0,    # minimum CE premium — below this brokerage eats profit
    # IV Rank filter (Gate 3)
    "iv_rank_enabled":             True,
    "iv_rank_min":                 45.0,    # IV rank % threshold (0-100); sell only when IV is elevated
    "iv_rank_history_days":        30,      # days used to compute the CE premium range
    # Nifty regime filter
    "nifty_regime_filter_enabled": True,
    "nifty_regime_ema_weeks":      20,      # weekly EMA period for regime detection
    "nifty_regime_max_strength_pct": 3.0,  # if Nifty is > X% above EMA → strong bull → skip
    # Earnings / event blackout (NSE corporate calendar)
    "results_exclusion_enabled":   True,
    "results_exclusion_days":      5,       # skip tickers with results within ±N calendar days
    # Telegram
    "telegram_bot_token":          "",
    "telegram_chat_id":            "",
}
...
def fetch_option_chain(ticker: str) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    try:
        ...
        today_dt      = datetime.now(IST).date()   # FIX: was date.today() (UTC on Linux servers)
...
# ─────────────────────────────────────────────────────────────
#  CE HISTORICAL RANGE  (high + low for IV Rank computation)
# ─────────────────────────────────────────────────────────────
def get_ce_historical_range(
    ticker: str, strike: float, expiry_str: str, days: int = 30
) -> Tuple[Optional[float], Optional[float], int]:
    """
    Fetch NSE historical CE closing prices for `days` days.
    Returns (high, low, nse_calls).
    Both high and low are needed to compute IV Rank.
    Debug prints removed for production performance.
    """
    try:
        session = get_nse_session()
        if session is None:
        ...
        return max(ltps), min(ltps), 1

    except Exception as e:
        print(f"[CE Range] {ticker}: {e}")
        return None, None, 0


# Backward-compat alias used by legacy callers
def get_ce_historical_high(
    ticker: str, strike: float, expiry_str: str, days: int = 30
) -> Tuple[Optional[float], int]:
    high, _, calls = get_ce_historical_range(ticker, strike, expiry_str, days)
    return high, calls
...
# ─────────────────────────────────────────────────────────────
def nearest_round_strike_above(
    price: float, strike_map: Dict, min_oi: int = 0
) -> Optional[float]:
    # Apply minimum OI filter alongside LTP liquidity check
    liquid_above = {
        s: data for s, data in strike_map.items()
        if s > price
        and data["CE_ltp"] > 0
        and data["CE_oi"] >= min_oi
    }
    if not liquid_above:
        # Fallback: ignore OI but still require LTP > 0
        liquid_above = {s: d for s, d in strike_map.items()
                        if s > price and d["CE_ltp"] > 0}
    if not liquid_above:
    ...
    # FIX: use actual NSE interval for the price level instead of hardcoded 50
    interval = nse_strike_interval(price)
    round_in_top3 = [s for s in top3 if int(s) % interval == 0]
    if round_in_top3:
...
def check_surge_continuity(
    ...
    Doji candles (body < 0.1% of open) are treated as neutral — neither
    green nor red — so they never falsely disqualify a valid surge window.
    """
    green_count       = 0
    allowed_red_count = 0
    ...
    for i in range(start, end + 1):
        ...
        is_doji  = body_pct < 0.001   # FIX: < 0.1% body = doji, skip structural check
        if is_doji:
            continue
...
# ─────────────────────────────────────────────────────────────
#  NIFTY REGIME FILTER
#  If Nifty 50 is in a strong uptrend (price > 20W EMA by
#  more than `nifty_regime_max_strength_pct`%), CE selling
#  carries elevated breakout risk and is skipped.
# ─────────────────────────────────────────────────────────────
_nifty_regime_cache: Dict[str, Any] = {"date": None, "is_bull": False}

def is_strong_bull_regime(cfg: Dict) -> bool:
    """
    Returns True if Nifty is in a strong bull regime (skip CE selling).
    Result is cached for the trading day to avoid repeated yfinance calls.
    """
    if not cfg.get("nifty_regime_filter_enabled", True):
        return False

    today_str = datetime.now(IST).strftime("%Y-%m-%d")
    if _nifty_regime_cache["date"] == today_str:
        return _nifty_regime_cache["is_bull"]

    try:
        weeks      = int(cfg.get("nifty_regime_ema_weeks", 20))
        max_str    = float(cfg.get("nifty_regime_max_strength_pct", 3.0))
        # Fetch ~6 months of weekly Nifty data for stable EMA
        nifty = yf.Ticker("^NSEI").history(period="6mo", interval="1wk", auto_adjust=True)
        if nifty.empty or len(nifty) < weeks + 1:
            _nifty_regime_cache.update({"date": today_str, "is_bull": False})
            return False

        closes     = nifty["Close"].values
        ema_vals   = compute_ema(closes, weeks)
        cur_close  = float(closes[-1])
        cur_ema    = float(ema_vals[-1])

        if cur_ema <= 0:
            _nifty_regime_cache.update({"date": today_str, "is_bull": False})
            return False

        strength_pct = (cur_close - cur_ema) / cur_ema * 100
        is_bull      = strength_pct > max_str
        _nifty_regime_cache.update({"date": today_str, "is_bull": is_bull})
        print(f"[Regime] Nifty {cur_close:.0f}  20W-EMA {cur_ema:.0f}  "
              f"strength={strength_pct:.1f}%  bull_regime={is_bull}")
        return is_bull
    except Exception as e:
...
# ─────────────────────────────────────────────────────────────
#  NSE EARNINGS / EVENT BLACKOUT
#  Skip tickers with results or board meetings within ±N days.
#  Earnings cause gap-risk that invalidates the CE-selling thesis.
# ─────────────────────────────────────────────────────────────
_results_cache: Dict[str, Any] = {}

def has_upcoming_results(ticker: str, cfg: Dict) -> bool:
    """
    Returns True if an earnings/board/AGM event is within
    `results_exclusion_days` calendar days of today.
    Uses NSE event-calendar API via the existing session.
    Result is cached per ticker per day.
    """
    if not cfg.get("results_exclusion_enabled", True):
        return False

    cache_key = f"{ticker}_{datetime.now(IST).strftime('%Y-%m-%d')}"
    if cache_key in _results_cache:
        return _results_cache[cache_key]

    try:
        excl_days = int(cfg.get("results_exclusion_days", 5))
        today     = datetime.now(IST).date()
        from_dt   = today - timedelta(days=excl_days)
        to_dt     = today + timedelta(days=excl_days)

        session = get_nse_session()
        if session is None:
            _results_cache[cache_key] = False
            return False

        url = (
            f"https://www.nseindia.com/api/event-calendar"
            f"?index=equities"
            f"&fromDate={from_dt.strftime('%d-%m-%Y')}"
            f"&toDate={to_dt.strftime('%d-%m-%Y')}"
        )
        ...
        events = r.json()
        keywords = {"financial result", "board meeting", "dividend", "agm", "egm"}
        for ev in events:
            ...
            if any(kw in purpose for kw in keywords):
                print(f"[Results] {ticker}: event '{ev.get('purpose')}' "
                      f"on {ev.get('date')} — skipping")
                _results_cache[cache_key] = True
                return True

        _results_cache[cache_key] = False
        return False
    except Exception as e:
        print(f"[Results] {ticker} error: {e}")
        _results_cache[cache_key] = False
        return False
...
# ─────────────────────────────────────────────────────────────
#  IV RANK COMPUTATION
#  IV Rank = (current_CE_ltp - 30d_low) / (30d_high - 30d_low) * 100
#  Proxy measure: uses CE premium range as a stand-in for IV range.
#  Only sell when IV is elevated (rank >= iv_rank_min).
# ─────────────────────────────────────────────────────────────
def compute_iv_rank(
    ce_ltp: float,
    ce_30d_high: Optional[float],
    ce_30d_low:  Optional[float],
) -> Optional[float]:
    """
    Returns IV rank as a percentage (0–100), or None if data is unavailable.
    A rank >= 45 means current premium is in the upper half of its range —
    a good time to be a seller.
    """
    if ce_30d_high is None or ce_30d_low is None:
        return None
    spread = ce_30d_high - ce_30d_low
    if spread <= 0:
        return None
    rank = (ce_ltp - ce_30d_low) / spread * 100
    return round(max(0.0, min(100.0, rank)), 1)


# ─────────────────────────────────────────────────────────────
#  BREAKOUT INVALIDATION
#  Signals where current price has closed above surge_high for
#  2+ consecutive candles are thesis-invalidated.
#  Called once per scan run to keep signals.json clean.
# ─────────────────────────────────────────────────────────────
def auto_invalidate_breakouts(cfg: Dict) -> int:
    """
    Mark and remove signals where price has broken out above surge_high.
    Returns the count of invalidated signals.
    """
    ...
    for sig in signals:
        ...
        try:
            hist = get_price_history(ticker, 5)
            if hist is None or hist.empty:
                surviving.append(sig)
                continue

            recent_closes = hist["Close"].values[-2:]   # last 2 candles
            # Invalidate only when 2 consecutive closes are above resistance
            # (one close above can be a wick / noise)
            if len(recent_closes) >= 2 and all(c > resistance for c in recent_closes):
                print(f"[Invalidate] {ticker}: closed above surge_high "
                      f"Rs{resistance:.2f} for 2+ candles — removing signal")
                removed += 1
...
def check_surge_and_loss(ticker: str, cfg: Dict, job_id: str = "") -> Optional[Dict]:
    # ── NIFTY REGIME FILTER (checked once per scan via cache) ────────────────
    if is_strong_bull_regime(cfg):
        if job_id:
            job_log(job_id,
                f"{ticker} SKIP (Regime): Nifty in strong bull — CE selling risky",
                "fail")
        return None

    # ── EARNINGS / EVENT BLACKOUT ────────────────────────────────────────────
    if has_upcoming_results(ticker, cfg):
        if job_id:
            job_log(job_id,
                f"{ticker} SKIP (Results blackout): earnings/event within "
                f"±{cfg.get('results_exclusion_days', 5)} days",
                "fail")
        return None
...
def find_best_strike(candidate: Dict, cfg: Dict, job_id: str = "") -> Optional[Dict]:
    ...
    # FIX: pass min_oi from config so illiquid strikes are excluded.
    min_oi = int(cfg.get("min_ce_oi", 500))
    strike = nearest_round_strike_above(surge_high, strike_map, min_oi=min_oi)
    if strike is None:
        if job_id:
            job_log(job_id,
    ...
    # FIX: minimum CE LTP filter — below this brokerage eats the premium
    min_ce_ltp = float(cfg.get("min_ce_ltp", 10.0))
    if ce_ltp < min_ce_ltp:
        if job_id:
            job_log(job_id,
                f"{ticker} CE LTP Rs{ce_ltp:.2f} < min Rs{min_ce_ltp:.2f} (not worth selling)",
                "fail")
        return None
    ...
    if cfg.get("ce_above_historical_high", True):
        # FIX: use get_ce_historical_range to get both high and low for IV Rank
        ce_hist_high, ce_hist_low, nse_hist_calls = get_ce_historical_range(
        ...
        # Compute IV rank proxy from CE premium range
        iv_rank = compute_iv_rank(ce_ltp, ce_hist_high, ce_hist_low)

        # IV Rank gate (only applied when we have enough data to compute it)
        if cfg.get("iv_rank_enabled", True) and iv_rank is not None:
            min_iv_rank = float(cfg.get("iv_rank_min", 45.0))
            if iv_rank < min_iv_rank:
                if job_id:
                    job_log(job_id,
                        f"{ticker} SKIP (IV Rank {iv_rank:.1f}% < {min_iv_rank:.0f}%) "
                        f"— premium too low, avoid selling cheap IV",
                        "fail")
                return None
            if job_id:
                job_log(job_id,
                    f"{ticker} IV Rank {iv_rank:.1f}% >= {min_iv_rank:.0f}% ✅",
                    "info")
    else:
    ...
    if spread_intervals > 0:
        ...
        if hedge_strike is not None and hedge_ce_ltp > 0 and ce_ltp > hedge_ce_ltp:
            ...
            spread_max_profit  = spread_net_premium
            spread_max_loss    = round(spread_width - spread_net_premium, 2)
    ...
    return {
        ...
        "Surge_High":           candidate["surge_high"],
        ...
        "CE_30d_Low":           round(ce_hist_low,  2) if ce_hist_low  else "N/A",
        "CE_IV_Rank":           iv_rank,          # NEW: IV rank (0–100)
...
def check_ema_confirmed_after_breakdown(
    ...
    """
    Scan from breakdown_date+1 forward.  Return
    ...
    FIX 1: Start from breakdown_date + 1 candle (Gate 2 must be a separate candle
           after breakdown, not the breakdown candle itself).
    FIX 2: avg_vol_20d uses the 20 candles BEFORE the breakdown, not after.
    FIX 3: default vol ratio corrected to 1.2 (was 0.5, making Gate 2 too loose).
    FIX 4: history window is max(lookback_days, 60) so EMA has proper warmup.
    """
    ema_period    = int(cfg.get("ema_period", 9))
    # FIX 3: default changed from 0.5 → 1.2 to match Gate 1 and DEFAULT_CONFIG
    min_vol_ratio = float(cfg.get("min_breakdown_volume_ratio", 1.2))
    recency_days  = int(cfg.get("surge_recency_days", 5))

    # FIX 4: always fetch enough data so EMA warmup is stable
    history_needed = max(int(cfg.get("lookback_days", 45)), 60)
    hist = get_price_history(ticker, history_needed)
    if hist is None or hist.empty:
    ...
    # Find the index of the breakdown candle itself
    bd_idx = next(
    ...
    # FIX 2: compute 20d avg volume from the 20 candles BEFORE the breakdown
    vol_window = vols[max(0, bd_idx - 20):bd_idx]
    avg_vol_20d = float(np.mean(vol_window)) if len(vol_window) > 0 else 0.0
    if avg_vol_20d <= 0:
        return False, None, None

    # FIX 1: start scanning from the candle AFTER the breakdown (bd_idx + 1)
    start_idx = bd_idx + 1
    ...
    for i in range(start_idx, len(closes)):
        if closes[i] < ema_values[i]:
            ...
            if vol_ratio >= min_vol_ratio:
                ...
                lookback_start = max(0, bd_idx - recency_days)
                if lookback_start < i:
...
def prune_stale_signals(signals: List[Dict], cfg: Dict) -> Tuple[List[Dict], int]:
    ...
    for s in signals:
        try:
            # FIX: prune based on Breakdown_Date (signal freshness) not Scanned_At
            # Using Scanned_At meant a re-scan could extend a stale signal's life.
            ref_str = s.get("Breakdown_Date") or s.get("Scanned_At")
            if not ref_str:
                fresh.append(s)
                continue
            # Breakdown_Date is "YYYY-MM-DD"; Scanned_At is "YYYY-MM-DD HH:MM:SS"
            fmt = "%Y-%m-%d" if len(ref_str) == 10 else "%Y-%m-%d %H:%M:%S"
            ref_dt = datetime.strptime(ref_str, fmt).replace(tzinfo=IST)
            if ref_dt >= cutoff:
                fresh.append(s)
            else:
                pruned += 1
...
def run_screen_job(tickers: List[str], cfg: Dict, job_id: str):
    ...
    # ── Invalidate broken-out signals before merging ─────────────────────────
    invalidated = auto_invalidate_breakouts(cfg)
    if invalidated:
        job_log(job_id, f"Auto-invalidated {invalidated} broken-out signal(s)", "warn")
...
class ConfigUpdate(BaseModel):
    ...
    # Strike quality filters
    min_ce_oi:                   Optional[int]   = None
    min_ce_ltp:                  Optional[float] = None
    # IV Rank filter
    iv_rank_enabled:             Optional[bool]  = None
    iv_rank_min:                 Optional[float] = None
    iv_rank_history_days:        Optional[int]   = None
    # Nifty regime filter
    nifty_regime_filter_enabled:    Optional[bool]  = None
    nifty_regime_ema_weeks:         Optional[int]   = None
    nifty_regime_max_strength_pct:  Optional[float] = None
    # Earnings blackout
    results_exclusion_enabled:   Optional[bool]  = None
    results_exclusion_days:      Optional[int]   = None
