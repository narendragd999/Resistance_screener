...
# ─────────────────────────────────────────────────────────────
#  NSE SYMBOL RENAMES
#  Map old/alternate NSE symbols → current yfinance symbol.
#  Add an entry here whenever a ticker is renamed, merged, or
#  yahoo finance diverges from the NSE symbol.
# ─────────────────────────────────────────────────────────────
NSE_SYMBOL_MAP: dict = {
    # Renamed after AU Small Finance Bank branding change
    "AUBANK":       "AUFINANCIALS",
    # Post HDFC-merger entities (yfinance still resolves these fine
    # but routing through resolver future-proofs renames)
    "HDFCBANK":     "HDFCBANK",
    "ICICIBANK":    "ICICIBANK",
    "KOTAKBANK":    "KOTAKBANK",
    "AXISBANK":     "AXISBANK",
    ...
    "RBLBANK":      "RBLBANK",
    "YESBANK":      "YESBANK",
    # Add more renames below as needed:
    # "OLDNAME": "NEWNAME",
}

# Screener.in URL slug overrides (only needed when slug ≠ NSE symbol)
# Rule: replace spaces with hyphens, drop special chars.
# Add entries here whenever Screener.in URL differs from NSE symbol.
SCREENER_SLUG_MAP: dict = {
    # Aditya Birla group
    ...
    # Others with known slug differences
    ...
    "AUFINANCIALS": "AU-SMALL-FINANCE-BANK",
    ...
    # "NSESYMBOL": "screener-slug",
}


def _resolve_yf_symbol(ticker: str) -> str:
    """
    Return the correct yfinance symbol for a given NSE ticker.
    Appends .NS suffix. Handles renames via NSE_SYMBOL_MAP.
    Always use this instead of raw f"{ticker}.NS".
    """
    ticker = ticker.strip().upper()
    mapped = NSE_SYMBOL_MAP.get(ticker, ticker)
    # Safety: strip any suffix the caller may have already added
    mapped = mapped.replace(".NS", "").replace(".BO", "")
    return mapped + ".NS"


def _screener_slug(ticker: str) -> str:
    """Convert NSE symbol to Screener.in URL slug."""
    ticker = ticker.strip().upper()
    return SCREENER_SLUG_MAP.get(ticker, ticker)
...
def _analyze_ticker(ticker: str, fy_start: int, force: bool, include_other_income: bool) -> Dict:
    ...
    try:
        ...
        # Cap end date to today — future dates confuse yfinance and trigger
        # false "possibly delisted / no data found" errors.
        end_str   = dt_module.date.today().strftime("%Y-%m-%d")
        yf_symbol = _resolve_yf_symbol(ticker)

        # Use Ticker().history() — more reliable than yf.download() in
        # yfinance 1.x for NSE symbols; handles timezone issues gracefully.
        tk_obj    = yf.Ticker(yf_symbol)
        price_df  = tk_obj.history(start=start_str, end=end_str, auto_adjust=True)

        # history() returns tz-aware index — normalise to tz-naive UTC date
        if not price_df.empty and price_df.index.tz is not None:
            price_df.index = price_df.index.tz_convert("UTC").tz_localize(None)

        # Fallback: if Ticker().history() fails, retry with yf.download()
        if price_df is None or price_df.empty:
            import logging
            logging.warning(f"{yf_symbol}: Ticker().history() empty, retrying with yf.download()")
