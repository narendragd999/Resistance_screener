...
def get_ce_historical_high(ticker: str, strike: float, expiry_str: str,
    ...
    """
    Returns (historical_high_or_None, nse_calls_made).
    Uses correct NSE endpoint: /api/historicalOR/foCPV
    """
    try:
        session = get_nse_session()
        if session is None:
            print(f"[{ticker}] NSE session is None — cannot fetch CE hist")
            return None, 0

        expiry_dt = datetime.strptime(expiry_str, "%d-%b-%Y")
        # Expiry format must stay as "28-Apr-2026" (dd-Mon-yyyy) — exactly as NSE returns it
        exp_nse   = expiry_dt.strftime("%d-%b-%Y")   # e.g. "28-Apr-2026"
        year      = expiry_dt.year
        ...
        print(f"[{ticker}] NSE hist URL: {url}")

        time.sleep(random.uniform(1.0, 2.0))
        r = session.get(url, timeout=15)
        print(f"[{ticker}] NSE hist status: {r.status_code}")

        if r.status_code in (403, 429):
            print(f"[{ticker}] NSE hist blocked — resetting session")
            reset_nse_session()
            return None, 1

        if r.status_code != 200 or not r.text.strip().startswith("{"):
            print(f"[{ticker}] NSE hist bad response: {r.text[:200]}")
            return None, 1

        data = r.json()
        rows = data.get("data", [])
        print(f"[{ticker}] NSE hist rows: {len(rows)}")

        if not rows:
            print(f"[{ticker}] NSE hist — no rows returned (new strike / illiquid)")
            return None, 1

        print(f"[{ticker}] Sample row: {rows[0]}")

        ltps = []
        for row in rows:
            # Try both field names — print sample row will confirm exact key
            val = row.get("FH_CLOSING_PRICE") or row.get("FH_LAST_TRADED_PRICE") or \
                  row.get("close") or row.get("lastPrice") or 0
            try:
                v = float(val)
                if v > 0:
                    ltps.append(v)
        ...
        print(f"[{ticker}] CE prices extracted ({len(ltps)}): {ltps}")

        if not ltps:
            print(f"[{ticker}] NSE hist — all prices zero/null")
            return None, 1

        high = max(ltps)
        print(f"[{ticker}] ✅ CE 30d hist high = ₹{high}")
        return high, 1

    except Exception as e:
        print(f"[{ticker}] NSE CE hist exception: {e}")
        return None, 0
...
def find_best_strike(candidate: Dict, cfg: Dict) -> Optional[Dict]:
    ...
    if cfg.get("ce_above_historical_high", True):
        ...
        print(f"[{ticker}] CE 30d hist high = {ce_hist_high}  (NSE hist calls used: {nse_hist_calls})")
        ...
        if ce_hist_high is not None:
            ...
            # None = both yfinance and NSE historical failed
    ...
    signal = {
        ...
        "NSE_Hist_Calls":    nse_hist_calls,        # for call budget tracking
