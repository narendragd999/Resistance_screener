...
def save_to_csv(ticker, data_dict, folder="data"):
    ...
    for table_name, df in data_dict.items():
        # Clean table name: convert to lowercase, replace spaces and special characters with underscores
        clean_table_name = table_name.lower().replace(' ', '_').replace('&', '_').replace('__', '_')
        # Map specific table names to desired format
        if clean_table_name.startswith('profit_and_loss') or clean_table_name.startswith('profit_loss') or clean_table_name == 'profitandloss':
            clean_table_name = 'profit_loss'
        elif clean_table_name.startswith('quarterlyresults') or clean_table_name.startswith('quarterly_results') or clean_table_name == 'quarterlyresults':
            clean_table_name = 'quarterly_results'
        # Create file name in the format TICKER_table_name.csv
        file_key = f"{ticker}_{clean_table_name}"
