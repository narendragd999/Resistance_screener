...
def _sig_ema9_support(close, low_s, rsi_s, e9, vol, vol_sma, i,
    ...
    """
    9 EMA Support signal:
    - Yesterday's low touched/came within tolerance% of 9 EMA
    - Today's close is above 9 EMA (bounce)
    - RSI 50–72
    ...
    v       = float(vol.iloc[i])
    vs      = float(vol_sma.iloc[i]) if float(vol_sma.iloc[i]) > 0 else 1

    touched_ema  = lo_prev <= e9_prev * (1 + tolerance_pct / 100)
    above_ema    = p > e9_cur
    rsi_ok       = 50 <= r <= 72
    vol_ok       = v >= min_volume and v >= vs * 0.8
