...
def load_from_csv(ticker):
    ...
    try:
        if os.path.exists(yearly_file):
            ...
            if yearly_data.empty or quarterly_data.shape[1] < 2:
                yearly_data = None
...
def suggest_improving_tickers(tickers, force_refresh, min_qoq_quarters, min_yoy_years):
    ...
    for ticker in tickers:
        ...
        if quarterly_data is not None and not quarterly_data.empty and '' in quarterly_data.columns:
            try:
                ...
                if adjusted_net_profit is not None and len(adjusted_net_profit) >= min_qoq_quarters:
                    # Check specified quarters for ascending net profit
                    ...
                    if is_ascending(recent_profits):
                        ...
                        qoq_np_growth = round(growth_rates.mean(), 2)
                ...
                if revenue is not None and len(revenue) >= min_qoq_quarters:
                    # Check specified quarters for ascending revenue
                    ...
                    if is_ascending(recent_revenue):
                        ...
                        qoq_rev_growth = round(growth_rates.mean(), 2)
                    else:
                        qoq_rev_growth = 'Not ascending'
                else:
                    qoq_rev_growth = f'Need {min_qoq_quarters} quarters'

                print(f"{ticker} QoQ Net Profit: {adjusted_net_profit.tolist() if adjusted_net_profit is not None else 'None'}, Growth: {qoq_np_growth}, Harsh: {qoq_harsh}")
        ...
        if yearly_data is not None and not yearly_data.empty and '' in yearly_data.columns:
            try:
                ...
                if adjusted_net_profit is not None and len(adjusted_net_profit) >= min_yoy_years:
                    # Check specified years for ascending net profit
                ...
                if revenue is not None and len(revenue) >= min_yoy_years:
                    # Check specified years for ascending revenue
                ...
                print(f"{ticker} YoY Net Profit: {adjusted_net_profit.tolist() if adjusted_net_profit is not None else 'None'}, Growth: {yoy_np_growth}, Harsh: {yoy_harsh}")
        ...
        # Add to suggestions if all criteria are met
        if (isinstance(qoq_np_growth, (int, float)) and
            ...
            error_msg = 'Did not meet improvement criteria'
            if qoq_np_growth == 'Error' or yoy_np_growth == 'Error' or qoq_rev_growth == 'Error' or yoy_rev_growth == 'Error':
                error_msg = 'Processing error'
            elif not qoq_harsh or not yoy_harsh:
                error_msg = 'No harsh condition recovery'
            suggestions.append({
                'Ticker': ticker,
                'QoQ Net Profit Growth (%)': qoq_np_growth,
                'YoY Net Profit Growth (%)': yoy_np_growth,
...
def main():
    ...
    if 'min_qoq_quarters' not in st.session_state:
        st.session_state.min_qoq_quarters = 4
