...
def process_multiple_tickers(tickers, enable_same_quarter):
    screener_data = []
...
def main():
    ...
    st.write("Enter NSE tickers (comma-separated, e.g., RELIANCE,HDFCBANK) or upload a CSV file with a 'Ticker' column or tickers in the first column. "
             "The app scrapes Quarterly Results and Profit & Loss tables from Screener.in, applies Excel formulas, checks if latest results are historically highest "
             "and in ascending order (both raw and adjusted), and optionally compares Net Profit of the same quarter year-over-year (e.g., Mar 2025 vs. Mar 2024). "
             "Results are downloaded as CSV and copied to clipboard. View screener-like summary and a filtered table with only PASS criteria tickers. Debug logs are printed for TATAELXSI.")
    ...
    if st.button("Scrape and Process Data"):
        ...
        with st.spinner("Scraping and processing data for all tickers..."):
            st.session_state.screener_df = process_multiple_tickers(tickers, st.session_state.enable_same_quarter)
            st.session_state.quarterly_data = {}
            st.session_state.yearly_data = {}
            for ticker in tickers:
                ticker = ticker.strip().upper()
        ...
        if selected_ticker:
            ...
            if quarterly_data is not None and not quarterly_data.empty:
                ...
                with st.spinner(f"Processing quarterly data for {selected_ticker}..."):
                    ...
                    display_results(f"QOQ Highest Historical, Ascending, and Same Quarter Test - {selected_ticker}", {**qoq_results, **same_quarter_results})
            ...
            if yearly_data is not None and not yearly_data.empty:
                ...
                with st.spinner(f"Processing yearly data for {selected_ticker}..."):
                    ...
                    display_results(f"YOY Highest Historical and Ascending Test - {selected_ticker}", yoy_results)
