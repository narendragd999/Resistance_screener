...
def main():
    ...
    st.write("Enter NSE tickers (comma-separated, e.g., RELIANCE,HDFCBANK) or upload a CSV file with a 'Ticker' column or tickers in the first column. "
             ...
             "and in ascending order (both raw and adjusted), downloads as CSV, and copies to clipboard. View screener-like summary for all tickers "
             "and a filtered table with only PASS criteria tickers.")
        ...
        if selected_ticker:
            ...
            if quarterly_data is not None and not quarterly_data.empty:
                ...
                with st.spinner(f"Processing quarterly data for {selected_ticker}..."):
                    ...
                    display_results(f"QOQ Highest Historical and Ascending Test - {selected_ticker}", qoq_results)
